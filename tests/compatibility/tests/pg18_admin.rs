...
async fn test_pg18_transactions() {
    ...
    client
        .simple_query("CREATE SCHEMA pg18_txn;")
        .await
        .unwrap();
    client
        .simple_query("CREATE TABLE pg18_txn.accounts (id INT PRIMARY KEY, balance NUMERIC);")
        .await
        .unwrap();
    client
        .simple_query("INSERT INTO pg18_txn.accounts VALUES (1, 100), (2, 50);")
        .await
        .unwrap();
    ...
    client
        .simple_query("UPDATE pg18_txn.accounts SET balance = 90 WHERE id = 1;")
        .await
        .unwrap();
    client
        .simple_query("UPDATE pg18_txn.accounts SET balance = 60 WHERE id = 2;")
        .await
        .unwrap();
    client.simple_query("COMMIT;").await.unwrap();

    let msgs = client
        .simple_query("SELECT balance FROM pg18_txn.accounts WHERE id = 1;")
        .await
        .unwrap();
    ...
    client.simple_query("BEGIN;").await.unwrap();
    client
        .simple_query("UPDATE pg18_txn.accounts SET balance = 80 WHERE id = 1;")
        .await
        .unwrap();
...
async fn test_pg18_session_management() {
    ...
    // SET and SHOW
    client
        .simple_query("SET application_name = 'nodus_admin_test';")
        .await
        .unwrap();
    // SHOW not strictly required by standard but common
    // Just ensuring SET doesn't panic
    client
        .simple_query("SET statement_timeout = 60000;")
        .await
        .unwrap();
}
...
async fn test_pg18_rbac() {
    ...
    client
        .simple_query("CREATE SCHEMA pg18_rbac;")
        .await
        .unwrap();
    client
        .simple_query("CREATE TABLE pg18_rbac.secure_data (id INT PRIMARY KEY, secret TEXT);")
        .await
        .unwrap();

    // Grant and Revoke
    client
        .simple_query("GRANT SELECT ON TABLE pg18_rbac.secure_data TO test_user;")
        .await
        .unwrap();
    client
        .simple_query("REVOKE SELECT ON TABLE pg18_rbac.secure_data FROM test_user;")
        .await
        .unwrap();
}
